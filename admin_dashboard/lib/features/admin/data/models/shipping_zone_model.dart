class ShippingZoneModel {
  ...
  factory ShippingZoneModel.fromMap(Map<String, dynamic> map) {
    return ShippingZoneModel(
      code: (map['code'] as String? ?? '').trim().toLowerCase(),
      governorate: (map['governorate'] as String? ?? '').trim(),
      governorateEn: (map['governorateEn'] as String? ?? '').trim(),
  ...
  bool get hasCorruptedText {
    return _looksCorrupted(governorate) ||
        aliasesAr.any(_looksCorrupted) ||
        aliasesEn.any(_looksCorrupted);
  }
  ...
  static const List<ShippingZoneModel> egyptSeedData = [
    ShippingZoneModel(
      code: 'cairo',
      governorate: 'القاهرة',
      governorateEn: 'Cairo',
