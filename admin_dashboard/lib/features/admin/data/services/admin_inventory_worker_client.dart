...
class AdminInventoryWorkerClient {
  AdminInventoryWorkerClient({
  ...
  Future<AdminInventoryRestockResult> executeRestock({
    ...
    String? reason,
    List<String> requestIds = const <String>[],
  }) async {
    final path = '$_baseUrl/admin/products/$productId/restock';
    ...
    try {
      _logger.debug(
        'Sending worker inventory restock request.',
        context: {
          'traceId': traceId,
          'productId': productId,
          'delta': delta,
      ...
      return AdminInventoryRestockResult.fromJson(data);
    } on DioException catch (error) {
      final code = error.response?.statusCode;
      final workerMessage = _readWorkerMessage(error.response?.data);
      _logger.warning(
        'Worker inventory restock request failed.',
        context: {
          'traceId': traceId,
          'productId': productId,
          'path': path,
      ...
      if (code == 400) {
        final normalized = workerMessage?.toLowerCase() ?? '';
        if (normalized.contains('delta') &&
            normalized.contains('positive integer')) {
          throw AdminOperationFailedException(
            AdminLocaleController.globalT(
              'errors.worker.positiveDeltaOnly',
              params: {'traceId': traceId},
            ),
          );
        }
...
String? _readWorkerMessage(dynamic data) {
  if (data is Map) {
    ...
    final directMessage = normalized['message']?.toString();
    if (directMessage != null && directMessage.trim().isNotEmpty) {
      return directMessage.trim();
    }
    final directError = normalized['error']?.toString();
