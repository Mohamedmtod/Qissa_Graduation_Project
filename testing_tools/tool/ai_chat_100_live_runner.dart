...
Future<void> main(List<String> args) async {
  final scenarioGroup = _argValue(args, '--group') ?? '';
  final scenarioId = _argValue(args, '--scenario') ?? '';
  final full = args.contains('--full') || scenarioId.trim().isNotEmpty;
  final smoke = args.contains('--smoke') || !full;
  final workerFirst = !args.contains('--current');
  final failOnNeedsFix = !args.contains('--allow-needs-fix');
  final device = _argValue(args, '--device') ?? 'emulator-5554';
  final modeName = workerFirst ? 'worker_first_experiment' : 'current';
  final suiteLabel = scenarioId.trim().isNotEmpty
      ? 'scenario'
      : full
      ? 'full'
      : 'smoke';
  ...
  final legacyJsonlFile = File(
    '${logsDir.path}/ai_chat_100_${suiteLabel}_live_$timestamp.jsonl',
  );
  final legacyConsoleFile = File(
    '${logsDir.path}/ai_chat_100_${suiteLabel}_console_utf8_$timestamp.log',
  );
  ...
  Future<void> persistReports({required bool finalWrite}) async {
    ...
    final summary = StringBuffer()
      ...
      ..writeln('- Mode: `${suiteStart['mode'] ?? (smoke ? 'smoke' : 'full')}`')
  ...
  Future<void> handleLine(String line) async {
    ...
    if (event['type'] == 'scenario_result') {
      event = normalizeLiveScenarioResultSchema(
        ...
        mode: event['mode']?.toString() ?? modeName,
      );
    }
    events.add(event);
    ...
    if (event['type'] == 'scenario_result') {
      ...
      stdout.writeln(
        '[AI-100-LIVE] completed ${event['scenarioId']} => '
  ...
  stdout.writeln('Starting AI Chat 100 ${smoke ? 'smoke' : 'full'} run...');
  if (scenarioGroup.isNotEmpty) {
    stdout.writeln('Scenario group: $scenarioGroup');
  }
  if (scenarioId.trim().isNotEmpty) {
    stdout.writeln('Scenario: ${scenarioId.trim()}');
  }
  stdout.writeln('Fail on needs_fix: $failOnNeedsFix');
  stdout.writeln('Mode: ${workerFirst ? 'worker-first' : 'current'}');
  ...
  final process = await Process.start('flutter', <String>[
    ...
    '-d',
    device,
    '--dart-define=AI_CHAT_EXPERIMENT_MODE=$modeName',
    '--dart-define=AI_CHAT_100_FULL=${full ? 'true' : 'false'}',
    '--dart-define=AI_CHAT_100_FAIL_ON_NEEDS_FIX=${failOnNeedsFix ? 'true' : 'false'}',
    if (scenarioGroup.isNotEmpty)
      '--dart-define=AI_CHAT_100_SCENARIO_GROUP=$scenarioGroup',
    if (scenarioId.trim().isNotEmpty)
  ...
  stdout.writeln('AI Chat 100 run finished with exit code $exitCode.');
  stdout.writeln('Summary: ${summaryFile.path}');
